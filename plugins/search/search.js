...
function executeSearch(searchQuery) {
  $.getJSON(indexURL, function (data) {
    // Deduplicate pages by permalink
    var seenPermalinks = {};
    var uniquePages = [];
    $.each(data, function(index, page) {
      if (!seenPermalinks[page.permalink]) {
        seenPermalinks[page.permalink] = true;
        uniquePages.push(page);
      }
    });
    
    var fuse = new Fuse(uniquePages, fuseOptions);
  ...
  $.each(result, function (key, value) {
    ...
    if (fuseOptions.tokenize) {
      ...
      $.each(value.matches, function (matchKey, mvalue) {
        if (mvalue.key == "colors" || mvalue.key == "categories" || mvalue.key == "machines") {
